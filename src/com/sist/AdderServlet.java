 ...
 * 2. 그런 다음 사용자의 요청이나 응답에 대한 HttpServletRequest객체와 
 *    HttpServletResponse 객체를 만들게 됨.
 * 3. 그리고 난 후 Servlet의 doGet() 메서드나 doPost() 메서드를 호출하면서 이 객체들을 전달.
 * 4. 톰캣이 사용자의 요청에 대한 정보를 모든 HttpServletRequest 객체의 속성으로 담아
 *    메서드로 전달함. 따라서 각 HttpServletRequest에서 제공하는 메서드들은 매개변수로
 *    넘어온 객체들을 이용하여 사용자가 전송한 데이터를 받아오거나 응답을 할 수 있는 것임.
 * 
